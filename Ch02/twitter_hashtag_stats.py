...
# distribution of hashtags per tweet
import collections
import sys
...
if __name__ == '__main__':
    ...
    with open(fname, 'r') as f:
        ...
        print("{} tweets with at least one hashtag({}%)".format(tweets_with_hashtags,
                                            tweets_with_hashtags_percent))

        d = collections.OrderedDict(sorted(hashtag_count.items()))

        # NOTE: hashtag_count not sorted by number of hashtags
        #for tag_count, tweet_count in hashtag_count.items():
        for tag_count, tweet_count in d.items():
            if tag_count > 0:
                ...
                print("{} tweets with {} hashtags ({}% total, {}% elite)".format(tweet_count,
                        tag_count, percent_total, percent_elite))

# usage:
# python twitter_hashtag_stats.py user_timeline_sberman8.jsonl
